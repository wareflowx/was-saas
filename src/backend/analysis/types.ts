...
export type ABCProduct = {
  ...
  readonly cumulativeContribution: number // Cumulative percentage
...
export type DeadStockAnalysisResult = {
  ...
  readonly summary: {
    ...
    readonly criticalLevel: { count: number; tiedCapital: number }
    readonly warningLevel: { count: number; tiedCapital: number }
    readonly monitorLevel: { count: number; tiedCapital: number }
  }
  readonly analysisDate: Date
  readonly parameters: {
    readonly warehouseId: string
