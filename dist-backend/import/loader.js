    ...
    const insertMany = db.transaction((inventory) => {
        for (const inv of inventory) {
            try {
                ...
                stmt.run(id, warehouseId, inv.productId, inv.locationId || null, inv.quantity, inv.availableQuantity, inv.reservedQuantity, inv.lastReceivedAt || null, inv.lastShippedAt || null);
                inserted++;
            }
            catch (error) {
