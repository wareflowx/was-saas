  ...
  `);
    ...
    const insertPickings = db.transaction((pickings) => {
        for (const picking of pickings) {
            try {
                pickingStmt.run(picking.id, picking.orderId, picking.orderNumber, picking.customerId, picking.customerName, picking.warehouseId, picking.pickingNumber, formatDate(picking.assignedDate), null, // completed_date
                picking.status, picking.priority, picking.totalQuantity, picking.pickedQuantity, picking.picker || null);
                pickingsInserted++;
            }
            catch (error) {
    ...
    const insertLines = db.transaction((lines) => {
        for (const line of lines) {
            try {
                lineStmt.run(line.id, line.pickingId, line.warehouseId, line.productId, line.productSku, line.productName, line.locationCode, line.zoneName || null, line.quantity, line.pickedQuantity, line.unit, line.status);
                linesInserted++;
            }
            catch (error) {
  ...
  `);
    ...
    const insertReceptions = db.transaction((receptions) => {
        for (const reception of receptions) {
            try {
                receptionStmt.run(reception.id, reception.supplierId, reception.supplierName, reception.warehouseId, reception.receptionNumber, formatDate(reception.expectedDate), reception.receivedDate ? formatDate(reception.receivedDate) : null, reception.status, reception.priority, reception.totalQuantity, reception.receivedQuantity);
                receptionsInserted++;
            }
            catch (error) {
    ...
    const insertLines = db.transaction((lines) => {
        for (const line of lines) {
            try {
                lineStmt.run(line.id, line.receptionId, line.warehouseId, line.productId, line.productSku, line.productName, line.orderedQuantity, line.receivedQuantity, line.rejectedQuantity, line.unitPrice);
                linesInserted++;
            }
            catch (error) {
  ...
  `);
    ...
    const insertReturns = db.transaction((returns) => {
        for (const ret of returns) {
            try {
                returnStmt.run(ret.id, ret.orderId, ret.orderNumber, ret.customerId, ret.customerName, ret.warehouseId, formatDate(ret.returnDate), ret.status, ret.priority, ret.reason, ret.reasonLabel, ret.totalQuantity, ret.returnedQuantity, ret.totalAmount, ret.refundedAmount, ret.processor || null, ret.completedDate ? formatDate(ret.completedDate) : null);
                returnsInserted++;
            }
            catch (error) {
    ...
    const insertLines = db.transaction((lines) => {
        for (const line of lines) {
            try {
                lineStmt.run(line.id, line.returnId, line.warehouseId, line.productId, line.productSku, line.productName, line.quantity, line.unitPrice, line.totalPrice, line.condition, line.resolution);
                linesInserted++;
            }
            catch (error) {
